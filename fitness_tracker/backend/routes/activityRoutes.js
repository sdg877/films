...
router.post("/", async (request, response) => {
  try {
    ...
    const userId = request.user._id; // Assuming req.user contains the logged-in user

    const newActivity = { 
      user: userId, // Link the activity to the user
      date, 
      time, 
      activity, 
      duration, 
...
router.get("/", async (request, response) => {
  try {
    const userId = request.user._id; // Assuming req.user contains the logged-in user
    const activityAll = await Activity.find({ user: userId }); // Filter by the logged-in user
